...
use {crate::errors::TokenSwapError, spl_math::precise_number::PreciseNumber, std::fmt::Debug};
...
/// Helper function for mapping to TokenSwapError::CalculationFailure
...
pub trait CurveCalculator: Debug {
    ...
    fn validate_supply(&self, token_a_amount: u64, token_b_amount: u64) -> Result<()> {
        if token_a_amount == 0 {
            return Err(error!(TokenSwapError::EmptySupply));
        }
        if token_b_amount == 0 {
            return Err(error!(TokenSwapError::EmptySupply));
        }
