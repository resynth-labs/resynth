...
use anchor_spl::token::{Burn, Mint, Token, TokenAccount, Transfer, self};
...
pub fn execute(ctx: Context<WithdrawAllTokenTypes>, pool_token_amount: u64, minimum_token_a_amount: u64, minimum_token_b_amount: u64) -> Result<()> {
  ...
  let withdraw_fee: u128 =
    if ctx.accounts.fee_receiver.key() == ctx.accounts.lptoken.key() {
      ...
      swap_pool.fees.owner_withdraw_fee(u128::try_from(pool_token_amount).unwrap())
  ...
  let results = swap_pool.swap_curve
