...
pub struct SwapAToB<'info> {
  ...
  pub source_vault_a: Box<Account<'info, TokenAccount>>,
  ...
  pub dest_vault_b: Box<Account<'info, TokenAccount>>,

  #[account(
    mut,
    token::mint = swap_pool.mint_b,
  )]
  pub dest_token_b: Box<Account<'info, TokenAccount>>,
...
pub fn execute(ctx: Context<SwapAToB>, amount_in: u64, minimum_amount_out: u64) -> Result<()> {
  ...
  // let (swap_token_a_info, swap_token_b_info, user_token_account_a_info, user_token_account_b_info) = match trade_direction {
  //   TradeDirection::AtoB => (ctx.accounts.source_token.to_account_info(), ctx.accounts.destination_vault.to_account_info(), ctx.accounts.source_token.to_account_info(), ctx.accounts.destination_token.to_account_info()),
  //   TradeDirection::BtoA => (ctx.accounts.destination_vault.to_account_info(), ctx.accounts.source_token.to_account_info(), ctx.accounts.destination_token.to_account_info(), ctx.accounts.source_token.to_account_info()),
  // };
  ...
  //     u128::try_from(ctx.accounts.destination_vault.amount).unwrap(),
  //     trade_direction,
  //     &swap_pool.fees,
  //   )
  ...
  //               to: ctx.accounts.host_fee_receiver.as_ref().unwrap().to_account_info().clone(),
  ...
  //     ctx.accounts.token_program.to_account_info().clone(),
  //     Transfer {
  //       from: ctx.accounts.destination_vault.to_account_info().clone(),
  //       to: ctx.accounts.destination_token.to_account_info().clone(),
  //       authority: ctx.accounts.authority.to_account_info().clone(),
  //     },
  //   ),
