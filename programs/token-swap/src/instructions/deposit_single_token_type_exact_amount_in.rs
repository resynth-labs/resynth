...
use anchor_spl::token::{Mint, Token, TokenAccount, MintTo, Transfer, self};

use crate::{constants::SWAP_POOL_ACCOUNT_SEED, curve::calculator::CurveCalculator, errors::TokenSwapError, types::TradeDirection};
use crate::state::*;
...
pub fn execute(ctx: Context<DepositSingleTokenTypeExactAmountIn>, source_token_amount: u64, minimum_pool_token_amount: u64) -> Result<()> {
    ...
    TradeDirection::AtoB => {
      token::transfer(
        CpiContext::new(
          ...
          Transfer {
            from: ctx.accounts.token_a.as_ref().unwrap().to_account_info().clone(),
    ...
    TradeDirection::BtoA => {
      token::transfer(
        CpiContext::new(
          ctx.accounts.token_program.to_account_info().clone(),
          Transfer {
            from: ctx.accounts.token_b.as_ref().unwrap().to_account_info().clone(),
